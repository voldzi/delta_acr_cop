...
vi.mock("./CopMap", async () => {
  ...
  return {
    CopMap: ({
      ...
      emergencyRoute?: { features?: unknown[]; routes?: Array<{ routeId?: string }> } | null;
        ...
        objects.length === 0 && emptyMessage
          ...
          : [
              ...
              onRequestIsochroneFromPoint
                ...
                : null,
              onActivateEmergencyRoute && emergencyRoute?.routes?.[1]?.routeId
                ? React.createElement(
                    "button",
                    {
                      "data-testid": "map-activate-alt-route",
                      key: "map-activate-alt-route",
                      onClick: () => onActivateEmergencyRoute(emergencyRoute.routes![1]!.routeId!),
                      type: "button"
                    },
                    "Použít alternativu"
                  )
...
describe("COP web dashboard", () => {
  ...
  it("requests an emergency route when chat sends a route map action", async () => {
        ...
        return jsonResponse({
          ...
          features: [
            ...
            },
            {
              ...
              properties: { label: "Alternativa přes sever", rank: 2, role: "alternative", routeId: "alternative" },
              type: "Feature"
            }
          ],
          generatedAt: "2026-05-19T08:00:00Z",
          ...
          routes: [
            ...
            },
            {
              distanceM: 3400,
              durationSeconds: 600,
              elevation: { gainM: 64, maxM: 590, minM: 516 },
              elevationProfile: [
                { distanceM: 0, elevationM: 516 },
                { distanceM: 1800, elevationM: 590 },
                { distanceM: 3400, elevationM: 535 }
              ],
              hazardsOnRoute: [{ label: "Nízká průjezdnost", severity: "medium" }],
              quality: { confidence: 0.8, engine: "valhalla", mode: "engine_route" },
              rank: 2,
              routeId: "alternative",
              traffic: {
              ...
              weatherOnRoute: { condition: "déšť", temperatureC: 9 }
            }
          ],
          traffic: {
            incidentCount: 1,
    ...
    expect(screen.getByTestId("cop-map").getAttribute("data-emergency-route-features")).toBe("2");
    ...
    const routeCallCount = fetchMock.mock.calls.filter(([url]) =>
      String(url).includes("/api/v1/routing/alternatives")
    ).length;
    fireEvent.click(screen.getByTestId("map-activate-alt-route"));
    await waitFor(() =>
      expect(screen.getByTestId("cop-map").getAttribute("data-emergency-route-message")).toContain(
        "Zásahová trasa: 3.4 km"
      )
    );
