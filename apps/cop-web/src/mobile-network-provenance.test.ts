...
describe("mobile network provenance", () => {
  ...
  it("translates basis codes without exposing raw provider constants", () => {
    expect(
      ...
      "předpočítaná mapa pokrytí, referenční OSM infrastruktura, bez potvrzeného operátorského stavu BTS, stacionární měření signálu ČTÚ"
    );
  });

  it("explains that BTS status is not operator-confirmed", () => {
    expect(mobileNetworkBtsStatusNotice(["NO_OPERATOR_BTS_STATUS"])).toBe(
