...
describe("COP map data helpers", () => {
  ...
  it("formats SIM route analysis sections for route selection details", () => {
    const card = formatEmergencyRouteSelectionCard(
      { label: "Alternativa přes sever", rank: 2, role: "alternative", routeId: "alt-route" },
      {
        ...
        routes: [
          ...
          },
          {
            distanceM: 4575,
            durationSeconds: 496,
            elevation: { gainM: 88, maxM: 642, minM: 518 },
            elevationProfile: [
              { distanceM: 0, elevationM: 520 },
              { distanceM: 1900, elevationM: 642 },
              { distanceM: 4575, elevationM: 536 }
            ],
            hazardsOnRoute: [{ label: "Popadané větve", severity: "medium" }],
            quality: { confidence: 0.84, engine: "valhalla", mode: "engine_route" },
            rank: 2,
            routeId: "alt-route",
            traffic: {
              delayPenaltySeconds: 90,
              incidentCount: 1,
              incidentsOnRoute: [{ label: "Uzavírka mostu", severity: "high" }],
              limitations: ["Hard exclusion uses Valhalla exclude_locations only for closure-like candidates."],
              sourceStatus: "ok"
            },
            warnings: ["Alternative route generated by applying Valhalla linear_cost_factors=2."],
            weatherOnRoute: { condition: "déšť", temperatureC: 8, windSpeedMs: 5.2 }
          }
        ],
        traffic: { incidentCount: 1, sourceStatus: "ok" },
        warnings: ["SIM generated one route variant."]
      }
    );

    expect(card.title).toBe("Alternativa přes sever");
    expect(card.subtitle).toContain("lze přepnout");
    expect(card.elevationProfile?.points).toHaveLength(3);
    expect(card.detailRows).toEqual(
      expect.arrayContaining([
        { label: "Délka", value: "4.6 km" },
        { label: "ETA", value: "8 min" },
        { label: "Počasí", value: "déšť · 8 °C · vítr 5.2 m/s" },
        { label: "Rizika", value: "1 rizika" }
      ])
    );
    expect(card.analysisSections?.map((section) => section.title)).toEqual(
      expect.arrayContaining(["Doprava", "Počasí", "Rizika", "Varování a degradace", "Alternativy"])
    );
    expect(card.analysisSections?.flatMap((section) => section.items)).toEqual(
      expect.arrayContaining([
        expect.stringContaining("Uzavírka mostu"),
        expect.stringContaining("Popadané větve"),
        expect.stringContaining("linear_cost_factors")
      ])
    );
  });
