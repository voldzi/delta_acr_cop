...
alternatives remain clickable but are rendered less prominently. The persistent
map route card stays intentionally compact and shows only distance, ETA, route
quality and a short traffic state. Clicking a route feature opens a dedicated
route detail panel, not the generic object popover. That detail contains
traffic, warnings, weather, hazards, elevation profile and available
alternatives. COP can promote a returned alternative to the active route locally
by reordering the already returned `routes[]`/`features[]`; this does not call
SIM again. If
`quality.mode` is
