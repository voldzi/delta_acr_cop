...
| 2026-05-21 | TAK Gateway production source v1 | Přidána volitelná server-side COP proxy pro SIM `tak-gateway-api`, canonical typ `TAK_COT_GATEWAY`, endpointy `/api/v1/tak/*`, krátká bbox cache, token pouze na backendu, Source Health a samostatná neveřejná skupina vrstev `Mobile units`/`Ground markers`/`Traffic tracks`. |
